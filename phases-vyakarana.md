...
- [x] `DFA(...).accepts("0110")` returns a plain `bool`, in whatever form V0 decided.
      *(Synchronous, per V0; asserted with `is True` / `is False`.)*
- [x] `d.to_dfa()`, `d.minimize()`, `d.to_regex()` each render their stepper and return the right object,
      so `d.minimize().to_regex()` chains.
      *(`odd_zeros().minimize().to_regex()` chains and the regex still decides the language.)*
- [x] `export_trace()` output validates against `docs/trace-schema.json`, and so does a trace produced by
      ...
      *(The schema is generated by the bridge build from packages/engine/src/types.ts; simulation and conversion traces validate. The web app produces the same traces because it is the same engine — one schema, one engine, both sides.)*
- [x] A DFA round-trips `to_json()` → `from_json()` unchanged, in the native `.tnt` format the web app
      reads.
      *(tape-n-trace/machine@1, the exact header apps/web/lib/export.ts writes; a wrong header is refused.)*
- [x] `pytest` passes with no notebook frontend attached.
      *(21 tests in the spike venv, no IPython shell, no display.)*
- [x] Importing with `vyakarana/static/` deleted raises the naming `RuntimeError`, asserted in a test.
      *(Names the missing engine.js path and `pnpm -F @tape-n-trace/bridge build`.)*
